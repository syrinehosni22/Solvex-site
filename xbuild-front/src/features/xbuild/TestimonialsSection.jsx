    ...
    <section ref={ref} style={{ padding:"100px 0", background:"linear-gradient(135deg,var(--color-dark, #121315) 0%,#1a1a25 100%)", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"0 24px", textAlign:"center", position:"relative" }}>
        ...
        <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:20, padding:"48px 48px 40px", opacity: visible?1:0, transition:"all 0.6s 0.3s" }}>
          <div style={{ fontSize:80, color:"var(--color-primary)", opacity:0.3, fontFamily:"Georgia,serif", lineHeight:0.5, marginBottom:24 }}>"</div>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div style={{ display:"flex", alignItems:"center", gap:16 }}>
              <div style={{ width:56, height:56, borderRadius:"50%", overflow:"hidden", background:"linear-gradient(135deg,var(--color-primary),#ff8c6b)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:24, flexShrink:0 }}>
                {testimonial.image
              ...
              <div style={{ textAlign:"left" }}>
                ...
                <div style={{ display:"flex", gap:2, marginTop:4 }}>
                  {Array.from({ length: testimonial.stars || 5 }).map((_, i) => <span key={i} style={{ color:"var(--color-primary)", fontSize:14 }}>★</span>)}
                </div>
              </div>
            </div>
        ...
        <div style={{ display:"flex", justifyContent:"center", gap:8, marginTop:32 }}>
          {testimonials.map((_, i) => (
            <button key={i} onClick={() => setActive(i)} style={{ width: i===active?28:8, height:8, borderRadius:4, background: i===active?"var(--color-primary)":"rgba(255,255,255,0.2)", border:"none", cursor:"pointer", transition:"all 0.3s" }} />
          ))}
        </div>
        <div style={{ display:"flex", justifyContent:"center", gap:12, marginTop:20 }}>
          ...
          <button onClick={() => setActive(a => (a+1)%testimonials.length)} style={{ width:44, height:44, borderRadius:"50%", background:"var(--color-primary)", border:"none", color:"#fff", cursor:"pointer", fontSize:18 }}>→</button>
        </div>
      </div>
    </section>
  );
