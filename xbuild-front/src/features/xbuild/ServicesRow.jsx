...
function IconDisplay({ icon, size = 28 }) {
...
export default function ServiceRow({ service, index, t, onContact, id }) {
  ...
  return (
    <div ref={ref} id={id} className="service-row" style={{
      display: "grid", gridTemplateColumns: "1fr 1fr", gap: 60, alignItems: "start",
      marginBottom: 90, scrollMarginTop: 130,
      opacity: visible ? 1 : 0, transform: visible ? "none" : "translateY(40px)", transition: "all 0.8s ease",
    }}>
      <div style={{ order: isEven ? 1 : 2, position: "relative" }}>
        <div style={{
          borderRadius: 20, overflow: "hidden", position: "relative", height: 480,
          background: service.image ? `url(${service.image}) center/cover no-repeat` : "linear-gradient(135deg,#1a1a25,#2a2a35)",
          display: "flex", alignItems: "center", justifyContent: "center",
        }}>
          {!service.image && <IconDisplay icon={service.icon} size={96} />}
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg, rgba(18,19,21,0.15), rgba(18,19,21,0.35))" }} />
        </div>
        <div style={{
      ...
      <div style={{ order: isEven ? 2 : 1 }}>
          ...
          <div className="service-features-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 28 }}>
            {service.features.map((f, fi) => (
              <div key={fi} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                <div style={{ width: 22, height: 22, borderRadius: "50%", background: accent, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1 }}>
        ...
        <button onClick={onContact}
          style={{ background: "transparent", color: "var(--color-dark, #121315)", border: "2px solid var(--color-dark, #121315)", padding: "13px 30px", borderRadius: 4, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", fontSize: 14, display: "inline-flex", alignItems: "center", gap: 10, transition: "all 0.3s" }}
