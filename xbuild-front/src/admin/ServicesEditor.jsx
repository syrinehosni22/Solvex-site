      ...
      <div style={{ display:"grid", gridTemplateColumns: editing ? "1fr 1fr" : "1fr", gap:20 }}>
        {editing && (
          <Card>
            ...
            <Field label="Ordre d'affichage" hint="Position dans la page Services (01, 02…)">
            ...
            <Field label={`Points forts (un par ligne) — ${lang === "fr" ? "Français" : "English"}`} hint="Affichés sous forme de liste à puces sur la page Services">
              <Textarea
                value={(form[`features_${lang}`] || []).join("\n")}
                onChange={e => patch(`features_${lang}`, e.target.value.split("\n").map(l => l.trim()).filter(Boolean))}
                placeholder={lang === "fr" ? "Études de faisabilité\nModélisation 2D/3D\nPlans d'exécution détaillés" : "Feasibility studies\n2D/3D modeling\nDetailed execution plans"}
                rows={4}
              />
            </Field>
