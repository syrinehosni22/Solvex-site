        ...
        {editing && (
          <Card>
            ...
            <Field label={`Description — ${lang === "fr" ? "Français" : "English"}`}>
              <Textarea value={form[`desc_${lang}`] || ""} onChange={e => patch(`desc_${lang}`, e.target.value)} placeholder={lang === "fr" ? "Décrivez ce service…" : "Describe this service…"} rows={3} />
            </Field>
            <Field label={`Points forts — ${lang === "fr" ? "Français" : "English"}`} hint="un par ligne">
              <Textarea value={(form[`features_${lang}`] || []).join("\n")} onChange={e => patch(`features_${lang}`, e.target.value.split("\n").map(l => l.trim()).filter(Boolean))} placeholder={lang === "fr" ? "Études de faisabilité\nModélisation 2D/3D\nPlans détaillés" : "Feasibility studies\n2D/3D modeling\nDetailed plans"} rows={4} />
            </Field>

            <Field label="Icône" hint="Uploadez un SVG ou PNG, ou saisissez un emoji">
              <SvgIconUploader value={form.icon} onChange={v => patch("icon", v)} size={56} />
            </Field>
            ...
            <div style={{ marginBottom: 16, padding: "14px", borderRadius: 12, background: "rgba(0,0,0,0.25)", border: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                <div style={{ width: 48, height: 48, borderRadius: 10, background: "rgba(245,91,31,0.1)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            ...
            items.map(doc => (
              <ListItem key={doc._id} active={editing?._id === doc._id}>
                <div style={{ display: "flex", gap: 10, alignItems: "center", flex: 1, minWidth: 0 }}>
                  <div style={{ width: 38, height: 38, borderRadius: 8, background: "rgba(245,91,31,0.08)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, overflow: "hidden" }}>
                    {doc.image ? <img src={doc.image} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <IconDisplay icon={doc.icon} size={22} />}
                  </div>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontWeight: 800, color: "#fff", fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{doc.title_fr || doc.title || "—"}</div>
                    <div style={{ color: "#666", fontSize: 11, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{doc.desc_fr || ""}</div>
                  </div>
                </div>
